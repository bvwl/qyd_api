...
async def post(item: Create = Body(..., description='创建数据')):
    ...
    try:
        ...
        return await server_group_crud.create(item.model_dump())
...
async def get(id: UUID = Path(..., description='ID')):
    ...
    """
    try:
        obj = await server_group_crud.get(id)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def gets(
    ...
    try:
        ...
        if res_count:
            count = await server_group_crud.get_count(
                name=name_filter,
                status=status,
            ...
            count = -1
        items = await server_group_crud.get_multi(
            name=name_filter,
            status=status,
            order_by=order_by,
            create_time_start=parse_time(create_time_start),
...
async def put(id: UUID = Path(..., description='主键ID'),
    ...
    """
    try:
        return await server_group_crud.update(id, item.model_dump(exclude_unset=True))
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete(id: UUID = Path(..., description='主键ID')):
    ...
    """
    try:
        ok = await server_group_crud.delete(id)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def post_or_put(item: Create = Body(..., description='创建或更新数据')):
    ...
    try:
        ...
        item.name = item.name.upper()
        return await server_group_crud.upsert(filter_kwargs, item.model_dump())
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
