...
async def post(item: Create = Body(..., description="创建数据")):
    try:
        ...
        return await server_country_crud.create(item)
...
async def get(id: UUID = Path(..., description="ID")):
    try:
        return await server_country_crud.get(id)
    except HTTPException as e:
        raise e
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def gets(
    ...
    try:
        name_filter = short_name.upper() if short_name else None
        return await server_country_crud.get_multi(
            short_name=name_filter,
            status=status,
            page=page,
            limit=limit,
            res_count=res_count,
...
async def put(
    ...
    try:
        return await server_country_crud.update(id, item)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except HTTPException as e:
        raise e
...
async def delete(id: UUID = Path(..., description="主键ID")):
    try:
        return await server_country_crud.delete(id)
    except HTTPException as e:
        raise e
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
