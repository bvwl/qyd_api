...
from app.models.project import ProjectBalance
...
class CRUD:
    ...
    async def create(self, item: Create) -> Out:
        res = await ProjectBalance.create(**item.model_dump())
        if not res:
            raise HTTPException(status_code=500, detail='创建失败')
        await res.fetch_related('account')
        return Out.model_validate(res)

    # 查询
    async def get(self, id: UUID) -> Out:
        res = await ProjectBalance.get_or_none(id=id)
        if not res:
    ...
    async def get_multi(self,
        ...
        num = len(res)
        for item in res:
            await item.fetch_related('account')
    ...
    async def upsert(self, item: Create) -> Out:
        record, created = await ProjectBalance.get_or_create(
        ...
        if not created:
            ...
            await record.save()
        await record.fetch_related('account')
        return Out.model_validate(record)


project_balance_crud = CRUD()
