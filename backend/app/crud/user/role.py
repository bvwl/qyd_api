...
class CRUD:
    ...
    async def create(self, item: Create) -> Out:
        is_exist = await Role.get_or_none(code=item.code)
        if is_exist:
            raise HTTPException(status_code=400, detail='角色标识已存在')
        res = await Role.create(**item.model_dump())
        if not res:
    ...
    async def get(self, id: UUID) -> Out:
        res = await Role.get_or_none(id=id)
        if not res:
    ...
    async def update(self, id: UUID, item: Update) -> Out:
        ...
        if 'code' in update_data:
            ...
            is_exist = await Role.get_or_none(code=new_code)
    ...
    async def upsert(self, item: Create) -> Out:
        record, created = await Role.get_or_create(
