...
class CRUD:
    ...
    async def create(self, item: Create) -> Out:
        is_exist = await ServerGroup.get_or_none(name=item.name)
        if is_exist:
    ...
    async def update(self, id: UUID, item: Update) -> Out:
        ...
        if 'name' in update_data:
            ...
            is_exist = await ServerGroup.get_or_none(name=new_name)
            if is_exist and is_exist.id != id:
                raise HTTPException(status_code=400, detail=f'分组名称 {new_name} 已被占用')
    ...
    async def upsert(self, item: Create) -> Out:
        record, created = await ServerGroup.get_or_create(
