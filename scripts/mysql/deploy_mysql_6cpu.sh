#!/bin/bash
# MySQL 单服务器主从部署脚本 - 6核CPU优化版
# 资源分配: 主库3核/8G, 从库1: 1.5核/4G, 从库2: 1.5核/4G
...
# 5. 启动主库
log_info "启动主库（端口${MASTER_PORT}，3核/8G）..."
docker run -d \
    --name mysql-master \
    --restart always \
    -p ${MASTER_PORT}:3306 \
...
for i in 1 2; do
    ...
    log_info "启动从库${i}（端口${port}，1.5核/4G）..."
...
echo ""
echo "资源分配："
echo "  主库: 3核/8G (端口3307)"
echo "  从库1: 1.5核/4G (端口3308)"
echo "  从库2: 1.5核/4G (端口3309)"
echo ""
echo "快速命令："
